...
def BRT(t, v, reach_values_):
    return jnp.minimum(v, reach_values_(t))


def BAT(t, v, avoid_values):
    return jnp.maximum(v, avoid_values(t))


def BRAT(t, v, reach_values, avoid_values):
    return jnp.maximum(jnp.minimum(v, reach_values(t)), avoid_values(t))


def BRAAT(t, v, vA, times, rBC, aBC):
    i = jnp.argmin(jnp.abs(t - times))  # nearest ix
    return jnp.minimum(jnp.maximum(v, aBC), jnp.maximum(rBC, vA[i, ...]))


def BRRT(t, v, V1, V2, times, rBC1, rBC2, lam=0.0):
    i = jnp.argmin(jnp.abs(t - times))
    return jnp.minimum(v, jnp.minimum(jnp.maximum(rBC2, V1[i, ...]), jnp.maximum(rBC1, V2[i, ...])))


# In[27]: INIT

u_bd, d_bd = 1.0, 0.0
diffgame = Point(u_bd=u_bd, d_bd=d_bd, alpha=0.0, control_mode="min", disturbance_mode="max")
...
grid = hj.Grid.from_lattice_parameters_and_boundary_conditions(
    hj.sets.Box(lbs - grid_pad, ubs + grid_pad), [grid_L for _ in range(2 * N)]
)
...
# NO TANH
def target_values_1(t):
    return jnp.linalg.norm(jnp.subtract(grid.states[..., :], c1), axis=-1) - rReach


def target_values_2(t):
    return jnp.linalg.norm(jnp.subtract(grid.states[..., :], c2), axis=-1) - rReach
...
init_values_avoid_ball = BAT(0.0, avoid_values_ball(0.0), avoid_values_ball)

## Define BC with post-processors (with values = bc(0.))
init_values_1 = target_values_1(0.0)
init_values_2 = target_values_2(0.0)
init_values_RR = jnp.maximum(target_values_1(0.0), target_values_2(0.0))
...
def reach_values_for_plotting(t):
    return jnp.minimum(target_values_1(0.0), target_values_2(0.0))


init_values_reachavoid_ball_1 = BRAT(0.0, target_values_1(0.0), target_values_1, avoid_values_ball)
init_values_reachavoid_ball_2 = BRAT(0.0, target_values_2(0.0), target_values_2, avoid_values_ball)
...
def solveplot(
    ...
    tf=TF,
    ntimes=5,
    reach_values=reach_values_for_plotting,
    avoid_values=avoid_values_ball,
    progress_bar=True,
    plot_no_value=False,
    plot_reach_solid=False,
    plot_avoid_solid=False,
    plot_bcs=False,
    plot_rBC=False,
    plot_aBC=False,
    one_shot=False,
    ...
    scaled_colors = np.vstack(
        (
            mpl.colormaps[cmap_name](np.linspace(0.0, 0.4, 256 - n_bins_high + offset)),
            mpl.colormaps[cmap_name](np.linspace(0.6, 1.0, n_bins_high - offset)),
        )
    )
...
## Ball - BAT
def BAT_ball_pp(t, v):
    return BAT(t, v, avoid_values_ball)


BAT_values_lin_ball_full, BAT_ball_lin_fig_last = solveplot(
    diffgame,
    init_values_avoid_ball,
    BAT_ball_pp,
    title="Point BAT Final",
    plot_avoid_solid=False,
    plot_no_value=False,
    plot_aBC=True,
    avoid_game=True,
...
## Ball - BRAAT
def BRAAT_ball_lin_l_pp_1(t, v):
    return BRAAT(t, v, BAT_values_lin_ball_full, times, target_values_1(t), avoid_values_ball(t))


BRAAT_values_1, BRAAT_fig_1_last = solveplot(
    diffgame,
    init_values_reachavoid_ball_1,
    BRAAT_ball_lin_l_pp_1,
    title=f"Point BRAAT Final",
    plot_reach_solid=False,
    plot_avoid_solid=False,
    plot_bcs=True,
    plot_no_value=False,
...
def BRAAT_ball_lin_l_pp_2(t, v):
    return BRAAT(t, v, BAT_values_lin_ball_full, times, target_values_2(t), avoid_values_ball(t))


BRAAT_values_2, BRAAT_fig_2_last = solveplot(
    diffgame,
    init_values_reachavoid_ball_2,
    BRAAT_ball_lin_l_pp_2,
    title=f"Point BRAAT Final",
    plot_reach_solid=False,
    plot_avoid_solid=False,
    plot_bcs=True,
...
init_values_rraa = BRRAAT(
    ...
    target_values_1(0.0),
    target_values_2(0.0),
    avoid_values_ball(0.0),
)
reach_values_stat, BRRAAT_fig = solveplot(
    diffgame,
    init_values_rraa,
    BRRAAT_pp,
    title="Point BRRAAT",
