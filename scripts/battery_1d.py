...
elif "raa_" in DIR_TAG:
    TASK_SOURCE = "F tg && G in_grid"

else:
    raise ValueError("Unknown TAG TASK {}".format(DIR_TAG))
    # TASK_SOURCE = "in_grid U tg"
    # TASK_SOURCE = "F tg && G in_grid"
    # TASK_SOURCE = "F tg && F port && G in_grid"

## Define environment dynamics
class PointBatterySimple(hj.ControlAndDisturbanceAffineDynamics):
    def __init__(self, 
                 u_bd=5., 
                 d_bd=0., 
                 charge_loss_rate=0.,
                 recharge_rate=0.,
                 port_center=jnp.array([0.]),
                 port_radius=0.2,
                 flow_x=0.,
                 control_mode="max", 
        ...
        self.flow_x = flow_x
        control_space = hj.sets.Ball(jnp.array([0]), u_bd)
        disturbance_space = hj.sets.Ball(jnp.array([0]), d_bd)
    ...
    #                         self.recharge_rate * jnp.exp((jnp.log(0.1/2)/(self.port_radius**2)) * jnp.linalg.norm(state[0] - self.port_center) ** 2) * (1/(1 + jnp.exp(5 + 50 * (charge - 1.))))
    #                       ])

    def control_jacobian(self, state, time):
        _, charge = state
        return jnp.array([[jnp.abs(charge)], [0.]])
    
    # Non-charge dependent (spec enforced)
    # def control_jacobian(self, state, time):
...
def main():
    ...
    # Y_BOTTOM = 0.0
    # Y_TOP = 2.0
    TARGET_CENTER = jnp.array([-0.25])
    TARGET_RADIUS = 0.15
    PORT_CENTER = jnp.array([0.25])
    PORT_RADIUS = 0.15

    U_MAX = 1.5
    FLOW_X = 0.0 if "flow" not in DIR_TAG else -0.5
    ...
    ## Define the grid
    if MODEL == "POINT":
        dyn = PointBatterySimple(u_bd=U_MAX, charge_loss_rate=CHARGE_LOSS_RATE, recharge_rate=RECHARGE_RATE,
...
def make_rooms(grid_dict: hj.Grid) -> dict[str, np.ndarray]:
    ...
    def sdf_target(pt):
        return jnp.linalg.norm(pt[0] - grid_dict["target_center"]) - grid_dict["target_radius"]
    
    def sdf_target_charged(pt):
        distance = jnp.linalg.norm(pt[0] - grid_dict["target_center"]) - grid_dict["target_radius"]
        charge = pt[1]
        # only consider target reached if charge > 0.1, sdf
        return jnp.maximum(distance, 0.1 - charge)

    def sdf_port(pt):
        return jnp.linalg.norm(pt[0] - grid_dict["port_center"]) - grid_dict["port_radius"]
    # just for plotting, not used in value computation

    def jit_rep_vmap(fn, rep: int, *args):
        return np.array(rep_vmap(fn, rep=rep)(*args))

    pos = np.array(grid.states)[:,:,:2]
    # only position affects value

    bb_sdf_target = -jit_rep_vmap(sdf_target, 2, pos)
    bb_sdf_target_charged = -jit_rep_vmap(sdf_target_charged, 2, pos)
    bb_sdf_port = -jit_rep_vmap(sdf_port, 2, pos)
    bb_sdf_grid_limits = jit_rep_vmap(sdf_grid_limits, 2, np.array(grid.states))
    ...
    rooms_bc_dict = {
        ...
        "target_charged": bb_sdf_target_charged,
        "port": bb_sdf_port,
        ...
        for i in range(batch_size):
            ...
            if (x >= grid_dict["lbs"][0] - grid_dict["grid_pad"][0]
                ...
                if highlight_reached:
                    ...
                    max_reach_value_2 = np.max(-(np.linalg.norm(states[i, :(step+1), 0] - grid_dict["port_center"]) - grid_dict["port_radius"]))
